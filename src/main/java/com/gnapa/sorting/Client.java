...
public class Client {
    ...
    public static void main(String[] args) {
        Integer[] arrayForBubbleSort = generateRandomArray(20000000, 100000);
        Integer[] arrayForInsertionSort = arrayForBubbleSort.clone();
        Integer[] arrayForMergeSort = arrayForBubbleSort.clone();
        Integer[] arrayForOptimizedMergeSort = arrayForBubbleSort.clone();
        ...
        sortAndTime(mergeSort, arrayForOptimizedMergeSort, true);

        QuickSort quickSort = new QuickSort();
        sortAndTime(quickSort, arrayForQuickSort);
    }

    private static void sortAndTime(MergeSort mergeSort, Integer[] randomArray, boolean b) {
        if (DEBUG) System.out.println(Arrays.toString(randomArray));
        long start = System.currentTimeMillis();
        mergeSort.optimizedMergeSort(randomArray);
